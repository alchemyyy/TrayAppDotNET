...
internal static class NightLightCloudStore
{
            ...
            dict)
    {
        ...
        using (XmlWriter writer = XmlWriter.Create(stream, writerSettings))
        {
            ...
            writer.WriteStartElement("NightLightKnownRVAs");
            foreach (KeyValuePair<string,
                             (int InitializeRVA, int SInstanceRVA, int SetTargetColorTemperatureRVA, int SetPreviewRVA)>
                         kvp in dict.OrderBy(kvp => kvp.Key, StringComparer.OrdinalIgnoreCase))
            {
                writer.WriteStartElement("Entry");
                writer.WriteAttributeString("Version", kvp.Key);
                writer.WriteAttributeString("Initialize", ToHex(kvp.Value.InitializeRVA));
                writer.WriteAttributeString("SInstance", ToHex(kvp.Value.SInstanceRVA));
                writer.WriteAttributeString("SetTargetColorTemperature", ToHex(kvp.Value.SetTargetColorTemperatureRVA));
                writer.WriteAttributeString("SetPreview", ToHex(kvp.Value.SetPreviewRVA));
        ...
        ParseKnownRVAs(byte[] xmlBytes)
    {
        ...
        if (root == null || root.Name.LocalName is not ("NightLightKnownRVAs" or "NightLightKnownRvas")) return result;

        foreach (XElement entry in root.Elements("Entry"))
        {
            string? version = (string?)entry.Attribute("Version");
            if (string.IsNullOrWhiteSpace(version)) continue;
            int initRVA = ParseRVA((string?)entry.Attribute("Initialize"));
            int sInstRVA = ParseRVA((string?)entry.Attribute("SInstance"));
            int setTempRVA = ParseRVA((string?)entry.Attribute("SetTargetColorTemperature"));
            int setPrevRVA = ParseRVA((string?)entry.Attribute("SetPreview"));
            result[version] = (initRVA, sInstRVA, setTempRVA, setPrevRVA);
        }
