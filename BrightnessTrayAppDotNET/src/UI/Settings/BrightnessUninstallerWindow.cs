...
public sealed class BrightnessUninstallerWindow : Window
{
    ...
    public BrightnessUninstallerWindow(string installDir, BrightnessInstallScope scope)
    {
        ...
        BrightnessInstallScope scope1 = scope;
        Title = L("Uninstaller_Title", "Uninstall BrightnessTrayAppDotNET");
        Width = 520;
        Height = 360;
        MinWidth = 420;
        MinHeight = 320;
        ...
        bool rounded = settings.EnableRoundedCorners;
        ...
        SettingsButton uninstall =
            TrayAppDotNETSettingsUI.Button(L("Uninstaller_UninstallButton", "Uninstall"), palette);
        uninstall.Padding = new Thickness(20, 8);
        SettingsButton cancel = TrayAppDotNETSettingsUI.Button(L("Uninstaller_Cancel", "Cancel"), palette);
        ...
        uninstall.Click += (_, _) =>
        {
            ...
            AppServices.Startup.RetargetShortcutIfPresent(exclude: scope1);
            ConfirmedUninstall = true;
            UninstallProcess = AppServices.Installation.RunUninstall(scope1, deleteSettings);
            Close();
        };
        ...
        chrome.Children.Add(BuildTitleBar(palette));
        Grid body = BuildBody(palette, rounded, buttons);
        Grid.SetRow(body, 1);
    ...
    private Grid BuildTitleBar(SettingsPalette palette)
    {
        ...
        TextBlock title =
            TrayAppDotNETSettingsUI.Text(L("Uninstaller_Title", "Uninstall BrightnessTrayAppDotNET"), palette, 13);
        title.VerticalAlignment = VerticalAlignment.Center;
        title.Margin = new Thickness(16, 0, 0, 0);
        titleBar.Children.Add(title);

        BrightnessUninstallerCaptionButton close = new(palette);
    ...
    private Grid BuildBody(SettingsPalette palette, bool rounded, StackPanel buttons)
    {
        ...
        TextBlock header = TrayAppDotNETSettingsUI.SectionHeader(
            L("Uninstaller_SectionHeader", "Uninstall BrightnessTrayAppDotNET"),
            palette);
        ...
        TextBlock description = TrayAppDotNETSettingsUI.DescriptionText(
            string.Format(
                L("Uninstaller_Description_Format",
                    "This will remove BrightnessTrayAppDotNET installed at \"{0}\" and its entry in Windows Settings > Apps. Choose what to do with your settings."),
                _installDir),
            palette,
    ...
    private static Border BuildOptionCard(RadioButton radio, string title, string description, SettingsPalette palette,
        bool rounded)
    {
        StackPanel text = new()
        {
            Children =
            {
                TrayAppDotNETSettingsUI.TitleText(title, palette),
                TrayAppDotNETSettingsUI.DescriptionText(description, palette),
            },
        };
...
internal sealed class BrightnessUninstallerCaptionButton : Border
{
    ...
    public BrightnessUninstallerCaptionButton(SettingsPalette palette)
    {
        _palette = palette;
        Width = 46;
        Height = 32;
