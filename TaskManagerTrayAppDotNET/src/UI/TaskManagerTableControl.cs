...
internal sealed class TaskManagerTableControl : DetailsGridControl
{
    ...
    private readonly Pen _expanderPen;
    ...
    private readonly double _iconGap;
    private readonly double _treeIndentWidth;
    private bool _sortDescending;
    private bool _externalSubscriptionsAttached;
#if DEBUG
    private string? _hotReloadPendingSelectedRowKey;
#endif
        ...
        TaskManagerWindowResources resources)
    {
        ...
        _fontFamily = new FontFamily(resources.AxamlTaskManagerTable.FontFamily);
        ...
        _groupTypeface = CreateTypeface(Math.Max(
            resources.AxamlTaskManagerTable.GroupFontWeightMinimum,
            CalculateFontWeight(_fontSize)));
    ...
    /// <summary>Detaches process-wide notifications while the page is outside the visual tree.</summary>
    internal void DetachExternalSubscriptions()
    {
        if (!_externalSubscriptionsAttached) return;
    ...
    public void SetRows(IReadOnlyList<TaskManagerTableRow> rows)
    {
        ...
        if (_hotReloadPendingSelectedRowKey != null && SelectedRow != null)
        {
            ...
            SelectedRowChanged?.Invoke(SelectedRow);
        }
#endif
    }
...
#if DEBUG
    /// <summary>Captures interactive table state before a hot-reload shell rebuild.</summary>
    internal TaskManagerTableControlHotReloadState CaptureHotReloadState()
    {
        string[] collapsedGroupKeys = new string[_collapsedGroupKeys.Count];
        _collapsedGroupKeys.CopyTo(collapsedGroupKeys);
        double[] columnWidths = (double[])_columnWidths.Clone();
        double[] baselineColumnWidths = new double[_schema.Columns.Length];
        for (int columnIndex = 0; columnIndex < _schema.Columns.Length; columnIndex++)
            baselineColumnWidths[columnIndex] = _schema.Columns[columnIndex].Width;

        return new TaskManagerTableControlHotReloadState(
            _selectedRowKey,
            _sortColumnIndex,
            _sortDescending,
            collapsedGroupKeys,
    ...
    /// <summary>Restores interactive state while letting changed AXAML column baselines win.</summary>
    internal void RestoreHotReloadState(TaskManagerTableControlHotReloadState state)
    {
        if ((uint)state.SortColumnIndex < (uint)_schema.Columns.Length)
        {
        ...
        for (int groupIndex = 0; groupIndex < state.CollapsedGroupKeys.Length; groupIndex++)
            _collapsedGroupKeys.Add(state.CollapsedGroupKeys[groupIndex]);

        int widthCount = Math.Min(
            _columnWidths.Length,
            Math.Min(state.ColumnWidths.Length, state.BaselineColumnWidths.Length));
        for (int columnIndex = 0; columnIndex < widthCount; columnIndex++)
        {
            double previousWidth = state.ColumnWidths[columnIndex];
            double previousBaseline = state.BaselineColumnWidths[columnIndex];
            double nextBaseline = _schema.Columns[columnIndex].Width;
            if (Math.Abs(nextBaseline - previousBaseline) >= 0.01) continue;
            if (Math.Abs(previousWidth - previousBaseline) < 0.01) continue;

            _columnWidths[columnIndex] = Math.Max(
    ...
    protected override void ApplyDetailsGridMetrics(double fontSize, double rowHeight)
    {
        ...
        _groupTypeface = CreateTypeface(Math.Max(
            _resources.AxamlTaskManagerTable.GroupFontWeightMinimum,
            fontWeight));
    }
    ...
    private void RebuildProjection(bool notifySelectionRemoval)
    {
        ...
        if (_selectedRowKey != null && selectedSourceRow == null)
        {
            ...
            if (string.Equals(
                    _selectedRowKey,
                    _hotReloadPendingSelectedRowKey,
    ...
    private void DrawSortCaret(DrawingContext context, double columnRight, double headerTop)
    {
        ...
        double halfWidth = _resources.AxamlProcessTable.PaintedSortCaretHalfWidth;
        double halfHeight = _resources.AxamlProcessTable.PaintedSortCaretHalfHeight;
    ...
    {
        double nextWidth = Math.Max(_schema.MinimumColumnWidth, width);
    ...
    private Typeface CreateTypeface(int fontWeight) =>
        new(_fontFamily, FontStyle.Normal, (FontWeight)fontWeight);
...
#if DEBUG
/// <summary>Interactive generic-table state retained across a Debug hot-reload rebuild.</summary>
internal readonly record struct TaskManagerTableControlHotReloadState(
    string? SelectedRowKey,
    int SortColumnIndex,
    bool SortDescending,
    string[] CollapsedGroupKeys,
    double[] ColumnWidths,
    double[] BaselineColumnWidths);
#endif
