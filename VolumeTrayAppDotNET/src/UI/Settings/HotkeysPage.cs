...
public sealed partial class VolumeSettingsWindow
{
        ...
        SettingsPalette p)
    {
        ...
        addButton.Click += (_, _) =>
        {
            ...
            _settings.Hotkeys.Add(new VolumeHotkeyBinding
            {
                Action = action,
        ...
        void Refresh()
        {
            VolumeHotkeyApplyResult? applyResult = null;
            try { applyResult = AppServices.HotkeyService?.Apply(_settings.Hotkeys); }
            catch (Exception ex) { TADNLog.Log($"VolumeSettingsWindow.Hotkeys.Apply: {ex.Message}"); }

            entries.Children.Clear();
            foreach (VolumeHotkeyBinding binding in _settings.Hotkeys
    ...
    private Border BuildHotkeyEntryCard(
        VolumeHotkeyAction action,
        VolumeHotkeyBinding binding,
        VolumeHotkeyApplyResult? applyResult,
        Action refresh,
        SettingsPalette p)
    {
